...
if __name__ == "__main__":
    ...
    # Local environment: determine number of workers from CPU cores
    try:
        n_workers = int(os.environ.get("N_CPU_WORKERS", os.cpu_count() or 1))
        if n_workers <= 0:
            n_workers = 1
    except Exception:
        n_workers = 1

    print(f"[MAIN] Local setting: Using {n_workers} worker(s)", flush=True)
    print(
